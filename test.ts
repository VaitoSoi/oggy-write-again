...
const envReg = /^env=(.+)$/
const yamlReg = /^yaml=(.+)$/
console.log(process.argv, envReg.test(process.argv[2]))
if (!process.argv[2] || process.argv[2] == 'env' || envReg.test(process.argv[2])) {
        ...
        !process.argv[2] || process.argv[2] == 'env'
            ...
            : (
                envReg.test(process.argv[2])
                    ? (envReg.exec(process.argv[2]) ?? ['', 'config.env'])[1]
                    : 'config.env'
            )
    ...
    const yaml = parse(readFileSync(path, { encoding: 'utf-8' }))
    console.dir(yaml, { depth: null })
    config = new YAML(yaml)
}
